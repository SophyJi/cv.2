...
void Matrix_fill_border(Matrix* mat, int value) {
  ...
  for (int i = ((mat -> width * mat -> height) - 1); i > ((mat -> width * mat 
      -> height) - mat -> width - 1); --i){
    mat -> data[i] = value;
  }
  for (int i = 0; i < (mat -> width * mat -> height); i += mat -> width){
    mat -> data[i] = value;
  }
  for (int i = (mat -> width - 1); i < (mat -> width * mat -> height); 
      i += mat -> width){
    mat -> data[i] = value;
  }
}
...
int Matrix_column_of_min_value_in_row(const Matrix* mat, int row,
  ...
  int min = Matrix_min_value_in_row(mat, row, column_start, column_end);
  for (int i = (row * mat -> width) + column_start; i < (row * mat -> width) + 
        column_end; ++i){
    if (mat -> data[i] == min){
      return (i - (row * mat -> width));
    }
  }
  return 0;
}
