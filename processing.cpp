...
void compute_vertical_cost_matrix(const Matrix* energy, Matrix *cost) {
  ...
  for (int i = 1; i < Matrix_height(cost); i++) {
    for (int j = 0; j < Matrix_width(cost); j++) {
      if(j == 0) {
        *Matrix_at(cost, i, j) = *Matrix_at(energy, i, j) + Matrix_min_value_in_row
        (cost, i - 1, j, j + 2);
      ...
      // if(j > 0 && j < cost -> width - 1) {
      //   *Matrix_at(cost, i, j) = *Matrix_at(energy, 
      //   i, j) + Matrix_min_value_in_row(cost, i - 1, j - 1, j + 1);
      // } else if (j == cost -> width - 1) {
      //   *Matrix_at(cost, i, j) = *Matrix_at(energy, 
      //   i, j) + Matrix_min_value_in_row(cost, i - 1, j - 1, j);
      // } else {
      //   *Matrix_at(cost, i, j) = *Matrix_at(energy, 
      //   i, j) + Matrix_min_value_in_row(cost, i - 1, j, j + 1);
      // }
    }
  }
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
  int col = Matrix_column_of_min_value_in_row
  (cost, Matrix_height(cost) - 1, 0, Matrix_width(cost) - 1);
  // int* ptr = &bottom_min;
  // int col = Matrix_column(cost, ptr);
...
void seam_carve_width(Image *img, int newWidth) {
  ...
  while (Image_width(img) > newWidth) {
    Matrix *mat_e = new Matrix;
    Matrix *mat_c = new Matrix;
    compute_energy_matrix(img, mat_e);
    compute_vertical_cost_matrix(mat_e, mat_c);
    int *seam = new int[Image_height(img)];
    find_minimal_vertical_seam(mat_c, seam);
